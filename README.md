...
Vite の開発サーバーが立ち上がり、ブラウザで編集できます。ビルド済み確認だけなら次も使えます。
...
`edit` は選択した project root の中だけを読み書きします。ファイル名を渡すと、そのファイルを初期状態で開きます。
...
手元で埋め込み用の Web 成果物だけ更新したい場合は `npm run build:desktop-assets` を使えます。
